...
StartConversion:
	conversion = cli.Cli{uploadConversion, conf.CliCMD, aggressive, waitForStatus}
	if attempts != 0 {
...
func ConvertByURLHandler(c *gin.Context) {
	...
	check := c.DefaultQuery("check", "false")
	format := c.DefaultQuery("format", "pdf")
	ext := c.Query("ext")

	source, err := converter.NewConversionSource(url, format, nil, check != "false", ext)
	if err != nil {
		s.Increment("conversion_error")
		if ravenOk {
...
func ConvertByFileHandler(c *gin.Context) {
	...
	check := c.DefaultQuery("check", "false")
	format := c.DefaultQuery("format", "pdf")
	source, err := converter.NewConversionSource("", format, file, check != "false", ext)
