...
func uploadToS3(awsConf AWSS3, contentType string, b []byte) error {
...
func (c UploadConversion) Upload(contentType string, b []byte) (bool, error) {
	if c.AWSS3.S3Bucket == "" || c.AWSS3.S3Key == "" {
		return false, nil
	}

	if err := uploadToS3(c.AWSS3, contentType, b); err != nil {
		return false, err
	}
