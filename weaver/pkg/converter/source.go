...
func uncheckedSource(s *ConversionSource, uri string) {
	s.URI = uri
...
func NewConversionSource(uri, format string, body io.Reader, checkUri bool, ext string) (*ConversionSource, error) {
...
func (s ConversionSource) GetConversionFormat() string {
	if s.Format == "pdf" {
		return "pdf"
	}
...
func (s ConversionSource) ContentType() string {
	if s.Format == "pdf" {
